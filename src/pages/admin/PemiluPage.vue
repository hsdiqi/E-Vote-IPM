<template>
  <div class="space-y-5 animate-fade-in">
    ...
    <BaseTable
      ...
      :data="list"
      :loading="loading"
      empty-title="Belum ada pemilu"
      empty-desc="Klik tombol Tambah Pemilu untuk memulai"
    >
    ...
    <BaseModal
      v-model="showModal"
      :title="editItem ? 'Edit Pemilu' : 'Tambah Pemilu'"
      size="md"
    >
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <BaseInput
          ...
          label="Prefix"
          id="pemilu-prefix"
          placeholder="contoh: BEM2025"
          required
          :error="errors.prefix"
          hint="Prefix unik untuk identifikasi pemilu"
        />
        ...
        <BaseInput
          v-model.number="form.max_choices"
        ...
        <div v-if="editItem" class="flex items-center gap-3">
          ...
          <span
            ...
            >{{ form.is_active ? "Pemilu Aktif" : "Pemilu Nonaktif" }}</span
          >
        </div>
      </form>
      <template #footer>
...
const handleSubmit = async () => {
  ...
  if (!editItem.value && !form.prefix.trim()) {
    errors.value.prefix = "Prefix wajib diisi";
    return;
  }
  ...
  try {
    if (editItem.value) {
      const { data } = await pemiluService.update(editItem.value.id, {
        name: form.name || null,
        start_time: toRFC3339(form.start_time) || null,
        end_time: toRFC3339(form.end_time) || null,
...
const fetchPemilu = async () => {
  loading.value = true;
  try {
    const pemilu = await pemiluService.getAll();
    list.value = pemilu || [];
  } catch (err) {
    list.value = [];
